...
def collect_pset_keys_from_project_documents(account_id: str, project_id: str, document_ids: list[str]) -> list[str]:
    """Return sorted pset filter keys from selected Project Documents."""
    temp_paths: list[str] = []
    ...
    try:
        ...
        for document_id in clean_ids:
            ...
            try:
                model = ifcopenshell.open(ifc_path)
                for elem in get_candidate_products(model):
                    psets = elem and extract_element_data(elem, file_label="").get("psets", {})
                ...
                raise StorageError(f"PSet-Schlüssel konnten für '{doc.get('original_filename')}' nicht gelesen werden: {exc}") from exc
        return sorted(pset_keys)
    finally:
        cleanup_temp_paths(temp_paths)
