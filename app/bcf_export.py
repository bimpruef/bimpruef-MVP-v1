...
# ---------------------------------------------------------------------------
# Issue-based export API
# ---------------------------------------------------------------------------

def _issue_to_clash_dict(issue: dict) -> dict:
    """Convert a stored ProjectIssue dict into the clash-like structure used by the BCF builder."""
    elements = issue.get("elements") or {}
    e1 = elements.get("element_1") or {}
    e2 = elements.get("element_2") or {}
    return {
        "global_id_1": e1.get("global_id", ""),
        "type_1": e1.get("ifc_type", ""),
        "name_1": e1.get("name", ""),
        "file_label_1": e1.get("document_name", ""),
        "global_id_2": e2.get("global_id", ""),
        "type_2": e2.get("ifc_type", ""),
        "name_2": e2.get("name", ""),
        "file_label_2": e2.get("document_name", ""),
        "issue_title": issue.get("title", ""),
        "issue_description": issue.get("description", ""),
        "issue_priority": issue.get("priority", "normal"),
        "issue_status": issue.get("status", "open"),
    }


def create_bcf_zip_from_issues(issues: list, project_name: str = "BIMPruef Issues") -> bytes:
    """Build a BCF archive from stored ProjectIssue dicts."""
    clash_like = [_issue_to_clash_dict(issue) for issue in issues or []]
    return create_bcf_zip_from_clashes(clash_like, project_name=project_name)
