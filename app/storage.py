...
def _slot_ifc_path(session_id: str, slot: int) -> str:
    ...
    return os.path.join(get_session_dir(session_id), f"model_{slot}.ifc")


def _slot_meta_path(session_id: str, slot: int) -> str:
    """Pfad zur Metadatei mit dem Originaldateinamen."""
    return os.path.join(get_session_dir(session_id), f"model_{slot}_name.txt")
...
def get_session_slots(session_id: str) -> List[int]:
    ...
    for i in range(1, MAX_FILES_PER_SESSION + 1):
        if os.path.exists(os.path.join(session_dir, f"model_{i}.ifc")):
            slots.append(i)
    return slots
...
def get_session_file_paths(session_id: str) -> Dict[str, str]:
    session_dir = get_session_dir(session_id)
    return {
        "model_1": os.path.join(session_dir, "model_1.ifc"),
        "model_2": os.path.join(session_dir, "model_2.ifc"),
        "meta_1":  os.path.join(session_dir, "model_1_name.txt"),
        "meta_2":  os.path.join(session_dir, "model_2_name.txt"),
    }
...
def _clash_cache_path(session_id: str, slot_a: int, slot_b: int, tolerance: float) -> str:
    ...
    return os.path.join(get_session_dir(session_id), key)
