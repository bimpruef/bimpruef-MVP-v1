...
Routen:
  ...
  GET  /projects/{project_id}/model → Model-Modul (leitet zum Viewer weiter)
  POST /projects/{project_id}/model/upload → Datei hochladen (leitet zu viewer/upload weiter)
...
def project_dashboard(project_id: str):
    ...
    quick_links = (
        ...
        f'<a href="/viewer/?session_id={_e(sid)}" class="btn" style="text-decoration:none">🔍 3D-Viewer</a>'
        f'<a href="/viewer/clash/?session_id={_e(sid)}" class="btn" style="text-decoration:none">⚡ Clash-Analyse</a>'
        f'<a href="/viewer/list/?session_id={_e(sid)}" class="btn" style="text-decoration:none">📋 Elementliste</a>'
        f'<a href="/viewer/rulecheck/?session_id={_e(sid)}" class="btn" style="text-decoration:none">✅ Rule-Check</a>'
        f'</div>'
        f'</div>'
    )
