...
Primäre Struktur:
  ...
  /projects/{id}/clash           → Project-Clash-Analyse
  /projects/{id}/issues          → Project-Issues
...
Legacy-Routen:
  ...
  /session/{id}
  /objects/
  /compare-clashes/              → Redirect auf Projektübersicht
  /download-clashes-bcf/         → Redirect auf Projektübersicht
...
@app.get("/compare-clashes/")
def compare_clashes_legacy():
    """
    Legacy-Route.

    Die Clash-Analyse ist kein Session-/Viewer-Modul mehr.
    Neue Clash-Berechnungen laufen projektbezogen über /projects/{project_id}/clash.
    """
    return RedirectResponse("/", status_code=303)


@app.get("/download-clashes-bcf/")
def download_clashes_bcf_legacy():
    """
    Legacy-Route.

    BCF-Export gehört nicht mehr zur direkten Clash-Analyse.
    BCF wird über gespeicherte Issues exportiert.
    """
    return RedirectResponse("/", status_code=303)
